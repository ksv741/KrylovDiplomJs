...
const formSend = ( price ) => {
  ...
  statusMessage.style.cssText = 'padding-top: 10%; position: fixed;top: 0;left: 0;width: 100%;height: 100%;font-size: 43px; color: white; background: rgba(0,0,0,.5); z-index: 999;'; //обозначаем стили тексту
  let img = false; //показана ли картинка
  formsAll.forEach((form) => {
    document.body.addEventListener('click', (event) => {
      let target = event.target; // отлавливаем цель нажатия
        if(img){ // если сейчас показывается картинка
          statusMessage.remove(); // то удаляем ее
    ...
    form.addEventListener('submit', (event) => {
      ...
      // debugger;
      form.appendChild(statusMessage); // добавляем на страницу элемент с текстом о начале обработки формы
      ...
      if(form.closest('.popup-consultation')){
        body.user_quest = document.querySelector('.director-form>input').value;
      }
      ...
      post(body)
        .then((response) => { //при успешной отправке
            // statusMessage.textContent = successMessage; // устанавливаем текст об успешной отправке 
        ...
        .catch((error) => { // в случае ошибке
          ...
          // statusMessage.textContent = errorMessage; // устанавливаем текст об ошибке при отправке формы
          statusMessage.innerHTML = `<strong>Ужас ...</strong><br><br><br><img src="${errorImg}">`; // показываем картинку 
          img = true;
        });
    })
